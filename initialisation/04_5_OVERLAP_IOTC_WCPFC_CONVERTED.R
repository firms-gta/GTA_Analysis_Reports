print('IOTC and WCPFC overlap analyse converted')


CA_GRID_DOUBLE_DECLARATIONS_GROUPPED_GRID_IOTC_WCPFC_DATA_only_doubled_species <- converted_overlapping %>%
  dplyr::filter(declarant_names == "IOTC, WCPFC") %>%
  filter(source_authority %in% c("IOTC", "WCPFC")) %>%
...
CA_GRID_DOUBLE_DECLARATIONS_GROUPPED_GRID_IOTC_WCPFC_DATA_SUMMARY <- CA_GRID_DOUBLE_DECLARATIONS_GROUPPED_GRID_IOTC_WCPFC_DATA_only_doubled_species %>% 
...
CA_GRID_DOUBLE_DECLARATIONS_GROUPPED_GRID_IOTC_WCPFC_DATA_SUMMARY_COMP <- 
  ...
  mutate(Double = as.factor(ifelse(is.na(IOTC)| is.na(WCPFC)|IOTC == 0 | WCPFC == 0 , "SINGLE", "DOUBLE")))
...
plot_list <- map(species_doubled, function(species){
  p <- cowplot::plot_grid(
    ggplot() +
      ...
      geom_sf(data = CA_GRID_DOUBLE_DECLARATIONS_GROUPPED_GRID_IOTC_WCPFC_DATA_SUMMARY_COMP %>% 
                ungroup()  %>% 
...
walk2(plot_list, species_doubled, function(plot, species){
  ggsave(paste0("outputs/charts/overlapping/IOTC_WCPFC/species_doubled/converted/double_declaration_",species, "_charts_and_plots_converted.png"), plot, width = 10, height = 7, dpi = 300)
})
...
CA_IN_OVERLAPPING_ZONES_IOTC_WCPFC <- converted_overlapping %>% filter(source_authority!="CCSBT") %>% inner_join(CWP_GRIDS_IN_OVERLAPPING_ZONES_IOTC_WCPFC, by = c("geographic_identifier" = "CWP_CODE", "GRIDTYPE"))
...
write.xlsx(CA_IN_OVERLAPPING_ZONES_IOTC_WCPFC_SUMMARY_GRIDTYPE, "outputs/datasets/CA_IN_OVERLAPPING_ZONES_IOTC_WCPFC_SUMMARY_GRIDTYPE_converted.xlsx")
...
write.xlsx(CA_IN_OVERLAPPING_ZONES_IOTC_WCPFC_SUMMARY_SPECIES, "outputs/datasets/CA_IN_OVERLAPPING_ZONES_IOTC_WCPFC_SUMMARY_SPECIES_converted.xslx")
